...
def prepare_game():
    ...
    # if no argument passes, start with random word
    if len(sys.argv) == 1:
    ...
    # first check if secret word is integer, convert and use as seed for random
    # if integer, choose word from secret words file
    # next check if arg is valid 5-letter lowercase word and in valid_guesses.txt
    elif len(sys.argv) == 2:
        arg = sys.argv[1]
        if arg.isdigit():
            random.seed(int(arg))
            with open("secret_words.txt", "r", encoding="ascii") as secret_words_file:
                secret_words = [word.rstrip() for word in secret_words_file]
            secret_word = random.choice(secret_words)
        elif len(arg) == 5 and arg.islower() and arg in valid_words:
            secret_word = arg
        else:
...
def is_valid_guess(guess, valid_guesses):
    ...
    """
    return guess in valid_guesses and len(guess) == 5 and guess.islower()
...
def get_feedback(secret_word, guessed_word):
    ...
    secret_word_lst = list(secret_word)  # easier modification

    # correct letters in correct pos
    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i]:
            feedback[i] = CORRECT_COLOR
            secret_word_lst[i] = None

    # correct letters in wrong pos
    for i in range(NUM_LETTERS):
        if guessed_word[i] != secret_word[i] and guessed_word[i] in secret_word_lst:
            feedback[i] = WRONG_SPOT_COLOR
            secret_word_lst[secret_word_lst.index(guessed_word[i])] = None
